...
func TestCurrentPolicyLookupObscuresSFTPPasswordForAgent(t *testing.T) {
	...
	payload, err := protocol.ParsePayload[protocol.PolicyPushPayload](msg)
	require.NoError(t, err)
	assert.True(t, payload.Storage.RclonePassObscured)
...
func TestCurrentPolicyLookupKeepsPlaintextSFTPPasswordForCapableAgent(t *testing.T) {
	database := newRouterAssemblyDatabase(t)
	agent := createStorageTestAgent(t, database, "v.ps")
	systemInfo, err := json.Marshal(map[string]any{
		"capabilities": []string{protocol.CapabilityPolicyPlaintextRclonePass},
	})
	require.NoError(t, err)
	require.NoError(t, database.DB.Model(&db.Agent{}).Where("id = ?", agent.ID).Update("system_info", string(systemInfo)).Error)
	...
	payload, err := protocol.ParsePayload[protocol.PolicyPushPayload](msg)
	require.NoError(t, err)
	assert.False(t, payload.Storage.RclonePassObscured)
	assert.Equal(t, "clear-sftp-password", payload.Storage.RcloneConfig["pass"])
}
