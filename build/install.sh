...
if [[ -z "$MASTER_URL" || -z "$ENROLL_TOKEN" || -z "$AGENT_URL" || -z "$AGENT_SHA256" ]]; then
    usage
fi
...
if ! allowed_download_url "$MASTER_URL"; then
    echo "Master URL must use https, localhost, or 127.0.0.1"
    exit 1
fi

if ! allowed_download_url "$AGENT_URL"; then
    echo "Agent URL must use https, localhost, or 127.0.0.1"
    exit 1
fi

if [[ ! "$AGENT_SHA256" =~ ^[0-9a-fA-F]{64}$ ]]; then
