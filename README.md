...
- **数据库备份源**：支持 PostgreSQL / MySQL 逻辑 dump，dump 文件可进入快照或压缩包，详见 [数据库备份源](docs/database-backup-sources.md)。
- **备份产物命名**：压缩包备份可配置业务名、远端目录模板和文件名模板，详见 [备份产物命名策略](docs/backup-artifact-naming.md)。
...
- **Agent 版本感知与灰度升级**：Master 可识别 Agent 版本，支持按标签创建 canary + 批次式 Agent 升级任务，详见 [Agent 批量升级与灰度发布](docs/agent-rollout-upgrades.md)。
...
11. 需要升级多台 Agent 时，在 **节点管理** 中按标签或显式节点创建 **批量升级** 任务，先验证 canary，再按批次推进。
...
选择 Docker 容器后，Agent 会在备份执行前重新检查容器，并将该容器的 bind mount、named/anonymous volume 挂载点、可发现的 Compose 配置文件，以及 Compose 标签声明或项目目录中的 `.env` 环境文件解析成实际备份路径。环境文件内容不会进入任务元数据或日志。VaultFleet 不会默认备份 `/var/lib/docker` 整体、overlay/image layer、网络或镜像内容。
...
Web UI 的 **快照浏览** 恢复流程会把恢复计划拆成源节点、快照、目标节点、恢复模式、目标目录、选择路径和 Docker 来源。提交恢复前必须先执行预检；预检通过后，最终恢复按钮才会启用。预检只验证当前计划，不会创建 Agent 命令或任务历史。支持批量恢复的 Agent 会上报 `docker_multi_container_restore` 能力，单个恢复计划最多可选择 50 个 Docker 来源；旧 Agent 仍使用兼容的单选恢复。
...
2. 选择目标节点以及一个或多个 Docker 容器或 Compose 服务。页面按照快照元数据顺序显示最终执行顺序。
3. 执行预检，检查目标 Agent 能访问 Docker Engine、快照中存在 Docker 元数据，并提示容器名称、Compose identity、恢复路径和主机端口冲突。
4. 预检通过后提交恢复。VaultFleet 会合并并去重所有选中来源的恢复路径，只执行一次数据恢复，然后按顺序串行重建容器。
5. 单个容器重建失败不会阻止剩余容器；任务会记录每项结果，并以 `partial_success` 标记部分成功。任务详情可生成只包含失败项的新恢复计划，重试前必须重新预检。

恢复 Compose 服务时，Agent 会显式使用原项目目录和已备份的环境文件，避免 1Panel 等应用模板中的 `${CONTAINER_NAME}`、`${PANEL_APP_PORT_HTTP}` 等变量被替换为空字符串。若 Compose 文件引用变量但快照和目标目录都没有可用环境文件，预检会阻止执行；应补齐项目 `.env` 后重新备份，而不是忽略警告强行启动。

Docker 恢复预检是就绪性检查，不是完整拓扑保证。镜像层或镜像导入、registry 凭据、外部 Docker 网络、secrets、外部卷、容器依赖顺序、应用健康检查、数据库迁移和应用级一致性不属于恢复保证，仍需运维人员确认。预检通过后，如果目标环境在提交前发生变化，恢复任务仍可能失败，最终结果以任务历史为准。

预检需要目标 Agent 上报 `restore_preflight` 能力，多容器恢复还需要 `docker_multi_container_restore`。若页面提示缺少能力，请升级目标节点 Agent 并等待它重新连接 Master。回滚到旧 Agent 后，Master 和 Web UI 会自动退回单容器选择；旧的 `docker_source_id` API 字段继续兼容。
