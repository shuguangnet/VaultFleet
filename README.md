...
- **备份可恢复性验证**：可定期对最新 restic 快照执行 check、ls、抽样检查和可选小文件临时恢复测试，详见 [备份可恢复性验证](docs/recoverability-verification.md)。
...
7. 在 **快照浏览** 中选择源节点和快照，点击恢复后选择目标节点、恢复模式、目标目录或 Docker 来源，先执行恢复预检，再确认提交恢复任务。
8. 需要迁移到新节点时，确保新节点 Agent 在线且能访问相同存储；在恢复抽屉中直接选择新节点作为目标节点，不需要为了看到旧快照而先创建同仓库策略。
